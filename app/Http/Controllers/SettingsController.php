...
class SettingsController extends Controller
{
        ...
        switch ($section) {
            ...
            case 'alerts':
                ...
                foreach ($params['template_vars']['logs'] as $i => $log) {
                    if (\App\ActivityLog::NAME_APP_LOGS == $log || \App\ActivityLog::NAME_OUT_EMAILS == $log) {
    ...
    public function processSave($section, $settings)
    {
        ...
        return \Eventy::filter('settings.after_save', $response, $request, $section, $settings);
    }
    ...
    public function ajax(Request $request)
    {
        ...
        if ('error' == $response['status'] && empty($response['msg'])) {
